...
utils.configurator
==================
Configuration loader for OmniSim.

``Config`` merges the system-level defaults (``configs/system/system.yaml``)
with one or more dataset-specific YAML files.  Dataset values take precedence
over system defaults.  CLI overrides are applied on top.

Usage::

    config = Config(['configs/imdb/imdb.yaml'])
    es_index = config['es_index']
    threshold = config.get('threshold_similarity', 0.3)
"""
...
class Config(object):
    ...
    def __init__(self, config_file_list=None):
        ...
        self.internal_config_dict = self._load_config_files(['configs/system/system.yaml'])
    ...
    def _init_parameters_category(self):
        ...
        self.parameters['OpenAI'] = openai_arguments
        self.parameters['Dataset'] = dataset_arguments
        self.parameters['Simulation'] = simulation_arguments
    ...
    def _convert_config_dict(self, config_dict):
        """Coerce string values read from YAML into their native Python types.

        YAML sometimes loads numeric-looking values (e.g. list literals or
        booleans written as ``"True"`` / ``"False"``) as plain strings.  This
        method walks every key and attempts conversion in this order:

        1. **eval()** — safely evaluates the string as a Python literal.
           Only the result types ``int``, ``float``, ``bool``, ``list``,
           ``tuple``, and ``dict`` are accepted; anything else (e.g. an
           arbitrary expression) falls back to the raw string so that
           ``eval`` cannot be exploited for code injection.
        2. **Boolean strings** — ``"true"`` / ``"false"`` (case-insensitive)
           are converted to Python ``True`` / ``False`` when eval fails.
        3. **Fallback** — the value is kept as a plain string.

        Args:
            config_dict: Mutable dict of raw config values (modified in place).

        Returns:
            The same dict with values converted to their native types.
        """
    ...
    def __getitem__(self, item):
        """Return a config value by key, or ``None`` if the key is absent."""
        if item in self.final_config_dict:
            return self.final_config_dict[item]
        else:
            return None
    ...
    def get(self, item, default=None):
        """ Get a config value with a fallback default (mirrors dict.get behaviour) """
        val = self.final_config_dict.get(item)
