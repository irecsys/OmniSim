...
# define color for different status
log_colors_config = {
    'DEBUG': 'cyan',
    'WARNING': 'yellow',
    'ERROR': 'red',
...
def set_color(log, color, highlight=True):
    """ Set color in log outputs
        Args:
            * log: log message
            * color: pre-defined colors
    """
...
def init_logger(config):
    ...
    """
    init(autoreset=True)
    LOGROOT = config['log_path_outputs']
    dir_name = os.path.dirname(LOGROOT)
    ensure_dir(dir_name)

    log_filename = '{}-{}.log'.format(config['model'], get_local_time())

    log_filepath = os.path.join(LOGROOT, log_filename)
    ...
    # -------------------------------
    # Suppress OpenAI/Azure HTTP request INFO logs
...
def success(msg, *args, **kwargs):
    """ Define a status named as "SUCCESS"
        Args:
